...
- **GitHub 远端(私有) `github.com/yoursmistery-ui/hard-core-sk-HSK-Refinery`,两分支各司其职**: `main` = **开发工作区**(本仓库,只收 mod 成品;项目文档只留在本地 `.docs/`,与 `_tmp/`/`参考素材/`/`.trash/` 等一起 gitignore);`mods` = **orphan 部署快照**(独立历史,镜像游戏当前加载的 `RimWorld\Mods` 全量 mod、约 2.4G,排除 `Unified.xml` 因其 103MB 撞 GitHub 单文件 100MB 硬限)。同步节奏:日常改动落 `main` 后 `git push`;需要固化"as-deployed"全量环境时再在一次性 worktree 里重建 `mods` 快照分支并强推。`mods` 走共享对象库建分支可让整合包 blob 与 `main` 去重,只上传额外工坊 mod。
...
- **⚠ 门控"研究"必须拦在「进度层」,UI 层置灰一定被 ResearchTreeSK 队列绕过(2026-09-10 实例)**: ResearchTreeSK 的科研队列全链路不看可用性 —— ①`Tree` 点击入队调 `ResearchProjectDef.GetIncompleteParentsRecursive()`(**不检查 Available**,把锁着的上游一起入队);②队列推进直调 `ResearchManager.SetCurrentProject()`,**只判 `baseCost > 0`**;③`Node.Available` 由 ResearchTreeSK 自算,只含 `HiddenPrerequisitesCompleted`,**不含 `PrerequisitesCompleted`**、也不看 `CanStartNow`。且**进度入口不止一个**:`ResearchManager.ResearchPerformed` **不调用** `AddProgress`(自己写 `progress[currentProj]`),且它在进度满时**无条件**调 `FinishProject`,而 `FinishProject` **递归完成全部前置** → 只要下游能攒满进度,上游锁着的科技就被顺带完成(跳关)。**修法: 在 `ResearchPerformed` + `AddProgress` 两处 progress 漏斗做 Prefix 硬拦 + `WorkGiver_Researcher.HasJobOnThing` 后置(不派人干活) + 道具入口(`CompUseEffect_FinishRandomResearchProject`)**。
  **另(v5.2, 同日实测): 正因为 `Node.Available` 不含可见前置, "门控锁不会继承给下游节点"** —— 实例: 微电子学(门控)锁着, 其下游 研究技术 III(`Research_table_C1`, **故意不给蓝图的科研速度节点**)却是亮的、可点、可排队。修法=自己补**前置链传递闭包** `BlueprintGateDatabase.IsGateLockedOnPath()`(`prerequisites` + `hiddenPrerequisites` 递归, 逐帧记忆化 `_gateLockMemo`+`Time.frameCount`, 先占位 false 防成环); **遇到"已解锁但没研究完"的门控即停止传播**(那是正常前置, 别把下游一起灰掉)。树只在窗口 `PreOpen()` 调一次 `UpdateCaches` → 要即时刷新得反射 `Tree.Initialized`/`Tree.Nodes` 调 `Node.UpdateCaches`。注意终态 `Mods/Unified.xml` 是 **UTF-16**, `grep` 搜不到, 必须用 Python 读。
- **⚠ `CanStartNow` 这类"门禁属性"补丁只能 Postfix**:Prefix 里直接写 `__result = ...; return false;` 会把原版全部其余条件(requiredResearchBuilding / 研究台通电 / TechprintRequirementMet / PlayerMechanitorRequirementMet / AnalyzedThingsRequirementsMet / IsHidden / InspectionRequirementsMet)**丢掉 = 反而放宽门禁**。后置只会更严,永远安全。
- **⚠ 别改 `ResearchProjectDef.CanBeResearchedAt` 来拦劳动**:原版科研页签用它生成「为什么不能研究」的原因列表(`MainTabWindow_Research.lockedReasons`),在源头返 false 会**误报成 `MissingRequiredResearchFacilities`(缺少研究设施)**。改在 `WorkGiver_Researcher.HasJobOnThing` 收口,效果一样且不污染提示。
- **⚠ 也别拦 `ResearchManager.FinishProject` 本身**:阵营起始研究标签(`FactionDef.startingResearchTags`)、意识形态 `MemeDef.startingResearchProjects`、archonexus 搬家(`ResetAllProgress` 后用 `ClassicStart` 标签恢复玩家已有科技)全走它,拦了会吞掉这些正规来源。只对具体道具/入口收口。
- **`TryPatch` 式手工挂 Harmony 必须传参数类型 + `DeclaredMethod` 兜底**:name-only `AccessTools.Method` 可能命中**基类虚方法**,后置挂基类上对子类 override **不生效**(静默失效)。判断条件 `targetMethod.DeclaringType != targetType` 即回退 `AccessTools.DeclaredMethod`。
