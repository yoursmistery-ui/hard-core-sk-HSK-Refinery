...
namespace QualityBuilder
{
	public class ModInitializerComponent : MonoBehaviour
	{
        ...
        private static bool existingCompInjectionPerformed;
        ...
        public void Update()
		{
            ...
            // RimWorld 1.6: components are injected into ThingDef.comps at map init, but blueprints/frames that
            // already exist in a loaded save were deserialized without the comp (comps come from the save file,
            // not re-initialized from def.comps). Add the comp to those existing instances once while playing so
            // quality selection works on them too.
            if (modActive && Current.ProgramState == ProgramState.Playing && !ModInitializerComponent.existingCompInjectionPerformed)
            {
                ModInitializerComponent.existingCompInjectionPerformed = true;
                QualityBuilder.ensureCompsOnExistingBlueprintsAndFrames();
            }
		}
		...
		public void OnLevelWasLoaded()
		{
            ...
            ModInitializerComponent.existingCompInjectionPerformed = false;
        }
	}
}
