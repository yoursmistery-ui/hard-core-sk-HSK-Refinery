...
namespace SmartFarming
{
    public class MapComponent_SmartFarming : MapComponent
	{
		...
		int SimulateDay(int numOfDays, int simulatedGrowth, Zone_Growing zone, ThingDef plant, ZoneData zoneData, RimWorld.Planet.World world, int tile, List<string> simulationReport)
		{
			...
			growthToday = (int)(growthToday * PlantUtility.GrowthRateFactorFor_Temperature(plant, average));
			...
			if (logging && Prefs.DevMode)
			{
				simulationReport.Add("   - day: " + dayToSimulate + 
					...
					" | temperature: " + PlantUtility.GrowthRateFactorFor_Temperature(plant, average).ToStringPercent());
			}
		...
		void CalculateYield(Zone_Growing zone, ZoneData zoneData)
		{
			...
			//Calculate the yield
			var harvestYieldRaw = zone.plantDefToGrow?.plant?.harvestYield ?? 0;
			var fertilitySensitivity = zone.plantDefToGrow?.plant?.fertilitySensitivity ?? 0.5f;
			...
			zoneData.nutritionYield = zone.cells.Count * harvestYieldRaw * Current.gameInt.storyteller.difficulty.cropYieldFactor * zoneData.nutritionCache * (ModSettings_SmartFarming.useAverageFertility ? zoneData.fertilityAverage : zoneData.fertilityLow) * fertilitySensitivity;
		}
		...
		{
			if(map?.gameConditionManager != null) tempOffsetCache = map.gameConditionManager.AggregateTemperatureOffset();
			currentDay = GenDate.DayOfYear(Current.gameInt.tickManager.TicksAbs, latitude);
			if(tile != -1 && world?.grid?[tile] != null) baseTemperature = world.grid[tile].temperature;
		...
		public static void DrawFieldEdges(List<IntVec3> cells, Zone zone)
		{
				...
				mapComp.growZoneRegistry.TryGetValue(gZone.ID, out ZoneData zoneData))
			{
				...
				GenDraw.DrawFieldEdges(cells, color, null, renderQueue:2900);
			}
			GenDraw.DrawFieldEdges(cells, renderQueue:2900);
		}
	}
}
