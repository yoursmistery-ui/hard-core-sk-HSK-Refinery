    ...
    public class ZoneData : IExposable
    {
        ...
        public void Init(MapComponent_SmartFarming comp, Zone_Growing zone)
        {
            ...
            pettyJobsGizmo = new Command_Toggle
            {
                ...
                toggleAction = delegate() { noPettyJobs = !noPettyJobs; }
            };
            allowHarvestGizmo = new Command_Toggle
            {
                ...
                toggleAction = delegate() { allowHarvest = !allowHarvest; }
            };
            ...
            orchardGizmo = new Command_Toggle
            {
                ...
                toggleAction = delegate() { orchardAlignment = !orchardAlignment; }
            };
            UpdateGizmos();
        ...
        private static void SetSowModeMulti(List<Pair<Zone_Growing, ZoneData>> growing, SowMode mode, bool allow)
        {
            foreach (var v in growing)
                if (v.first != null && v.second != null)
                {
                    v.first.allowSow = allow;
                    v.second.sowMode = mode;
                    v.second.UpdateGizmos();
                }
        }

        public void SetSowModeMulti(MapComponent_SmartFarming comp, List<Pair<Zone_Growing, ZoneData>> toChange, SowMode? hardSet = null)
        {
            SoundDefOf.Click.PlayOneShotOnCamera(null);
            if (hardSet != null) sowMode = hardSet.Value;
            switch (sowMode)
            {
                case SowMode.Force:
                    SetSowModeMulti(toChange, SowMode.Off, false);
                    break;
                case SowMode.On:
                    SetSowModeMulti(toChange, SowMode.Smart, true);
                    foreach (var v in toChange)
                        comp.CalculateAll(v.first);
                    break;
                case SowMode.Smart:
                    SetSowModeMulti(toChange, SowMode.Force, true);
                    break;
                default:
                    SetSowModeMulti(toChange, SowMode.On, true);
                    break;
            }
        }

        private static void SetPriorityMulti(List<Pair<Zone_Growing, ZoneData>>  data, Priority priority)
        {
            foreach (var v in data)
                if (v.second != null)
                {
                    v.second.priority = priority;
                    v.second.UpdateGizmos();
                }
        }

        public void SetPriorityMulti(List<Pair<Zone_Growing, ZoneData>>  toChange, Priority? hardSet = null)
        {
            priority = priority != Priority.Critical ? ++priority : Priority.Low;
            SetPriorityMulti(toChange, priority);
        }
        ...
        public void MergeZones(List<Pair<Zone_Growing, ZoneData>> otherZones)
        {
            if (otherZones == null || otherZones.Count <= 1)
            {
                this.isMerged = true;
                return;
            }

            Zone_Growing first = null;

            foreach (var otherZone in otherZones)
            {
                if (otherZone.first == null) continue;
                if (first == null)
                {
                    first = otherZone.first;
                    continue;
                }
                var workingList = new List<IntVec3>(otherZone.first.cells);
                foreach (var cell in workingList)
                {
                    otherZone.first.RemoveCell(cell);
                    first.AddCell(cell);
                }
                Find.Selector.Deselect(otherZone.first);
            }
