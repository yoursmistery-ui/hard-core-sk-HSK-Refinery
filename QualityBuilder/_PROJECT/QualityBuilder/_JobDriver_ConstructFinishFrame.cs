...
namespace QualityBuilder
{
    ...
    public class _JobDriver_ConstructFinishFrame
    {
        ...
        public static void afterFinishToil(CompQualityBuilder cmp, Map curMap, LocalTargetInfo targetInfo)
        {
            try
            {
                AfterFinishToilInner(cmp, curMap, targetInfo);
            }
            catch (Exception e)
            {
                Log.Warning("QualityBuilder: afterFinishToil failed (frame likely de-spawned/map switched): " + e.Message);
            }
        }

        private static void AfterFinishToilInner(CompQualityBuilder cmp, Map curMap, LocalTargetInfo targetInfo)
        {
            if (cmp == null)
                return;
