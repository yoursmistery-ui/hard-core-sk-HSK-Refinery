...
namespace QuestBoardHSK
{
    ...
    {
        public const int MaxAgeDays = 60;   // 档案只存留半年(60 游戏日),过期每天低频清理
        public const int MaxRaids = 100;    // 兜底条数上限(防极端高频袭击撑爆存档)
        ...
        /// <summary>按"只存留半年"清理过期记录(列表最新在前,从首个未过期处整体截断)。</summary>
        private void PruneExpired()
        {
            int today = GenDate.DaysPassedSinceSettle;
            int cut = -1;
            for (int i = 0; i < raids.Count; i++)
            {
                if (today - raids[i].day <= MaxAgeDays)
                {
                    cut = i;
                    break;
                }
            ...
            if (cut < 0)
            {
                ...
                raids.RemoveRange(cut, raids.Count - cut);
        }

        public override void GameComponentTick()
        {
            // 档案只存留半年:每天低频清理一次过期记录(加载旧档后首个游戏日内生效)
