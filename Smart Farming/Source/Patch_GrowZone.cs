...
namespace SmartFarming
{
    ...
    static class Patch_GetGizmos
    {
        ...
        {
            ZoneData highestMode = zoneData;
            ZoneData highestPriority = zoneData;
            ZoneData basisZoneData = zoneData;


            List<Pair<Zone_Growing, ZoneData>> basisZones = new List<Pair<Zone_Growing, ZoneData>>() { new Pair<Zone_Growing, ZoneData>(thisZone, zoneData) };
            var selected = Find.Selector.selected;
            for (int i = selected.Count; i-- > 0;)
            {
                if (selected[i] is Zone_Growing growZone && comp.growZoneRegistry.TryGetValue(growZone.ID, out var localData) && localData != null && growZone != thisZone)
                {
                    if (localData.sowMode > highestMode.sowMode) highestMode = localData;
                    if (localData.priority > highestPriority.priority) highestPriority = localData;
                    basisZones.Add(new Pair<Zone_Growing, ZoneData>(growZone, zoneData));
                }
            }
            ...
            {
                defaultLabel = ("SmartFarming.Icon.SetAll".Translate() + highestMode.sowGizmo.defaultLabel.ToLower()),
                defaultDesc = highestMode.sowGizmo.defaultDesc,
                hotKey = KeyBindingDefOf.Command_ItemForbid,
                icon = basisZoneData.iconCache[highestMode.sowMode],
                action = () => zoneData.SetSowModeMulti(comp, basisZones, highestMode.sowMode)
            };
            yield return new Command_Action()
            {
                defaultLabel = ("SmartFarming.Icon.SetAll".Translate() + highestPriority.priorityGizmo.defaultLabel.ToLower()),
                defaultDesc = highestPriority.priorityGizmo.defaultDesc,
                icon = ResourceBank.iconPriority,
                action = () => zoneData.SetPriorityMulti(basisZones, highestPriority.priority)
            };
            if (!basisZones.NullOrEmpty())
            {
                yield return new Command_Action()
                {
                    ...
                    action = () => zoneData.MergeZones(basisZones)
                };
            }
    ...
    // Old patch nolonger works due to missing isSowing
    ...
    {
        static bool Prefix(ref bool __result, Map map, IntVec3 c, ThingDef plantDef)
        {
            if (oldPatchMethod)
            {
                ...
                {
                    var validPlant = map.thingGrid.ThingAt(c, plantDef) != null;

                    switch (zoneData.sowMode)
                    {
                        case SowMode.Smart:
                        {
                            __result = !validPlant && (zoneData.alwaysSow ? true : zoneData.minHarvestDayForNewlySown > -1);
                            return false;
                        }
                        case SowMode.Force:
                        {
                        ...
                        case SowMode.On:
                        {
                            return true; //Vanilla handling we dont care
                        }
                        case SowMode.Off:
                        {
                            __result = false;
    ...
    static class Patch_WorkGiver_GrowerSow
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            bool ran = false;
            bool next = false;
            const int length = 4; // how far from the final in the method to the first ldarg are we


            var watchedMethod = AccessTools.Method(typeof(PlantUtility), nameof(PlantUtility.GrowthSeasonNow), new[] { typeof(IntVec3), typeof(Verse.Map), typeof(Verse.ThingDef) });
            var prefixMethod = AccessTools.Method(typeof(Utilities), nameof(Utilities.GrothSeasonNowPrefix));
            ...
            for (var i = 0; i < original.Count; i++)
            {
                ...
                if (!ran)
                {
                    if (next)
                    {
                        ...
                        yield return (new CodeInstruction(OpCodes.Call, prefixMethod));
                        yield return (original[i]);

                        // should be the first ldarg for this method
                        for (int i2 = i - length; i2 < i; i2++)
                            yield return (original[i2]);
                        ran = true;
                    }
                    else if (inst.opcode == OpCodes.Call && inst.operand is MethodInfo mi && mi == watchedMethod)
                    {
                        next = true;
                        continue;
                    }
    ...
    static class AutoCutIfDying
    {
        ...
        {
            Map map = __instance.Map;
            if (__instance.def.plant.dieIfLeafless && compCache.TryGetValue(map.uniqueID, out MapComponent_SmartFarming mapComp) &&
