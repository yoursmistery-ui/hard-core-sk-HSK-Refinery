...
public static class CollectionExtensions
{
	...
	private static int[]? GetBuckets<TKey, TValue>(Dictionary<TKey, TValue> dictionary)
#if V1_6
		=> dictionary._buckets;
#else
		=> dictionary.buckets;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static Dictionary<TKey, TValue>.Entry[]? GetEntries<TKey, TValue>(Dictionary<TKey, TValue> dictionary)
#if V1_6
		=> dictionary._entries;
#else
		=> dictionary.entries;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static int GetBucketEntryIndex(int[] buckets, int hashCode)
#if V1_6
		=> buckets[hashCode % buckets.Length] - 1;
#else
		=> buckets[hashCode % buckets.Length];
#endif

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static bool IsValidEntryIndex<T>(int index, T[] entries)
	...
	public static ref TValue GetReference<TKey, TValue>(this Dictionary<TKey, TValue> dictionary, TKey key)
	{
		...
		if (buckets != null)
		{
			...
			var bucket = GetBucketEntryIndex(buckets, keyCode);
			var collisionCount = 0;
			while (IsValidEntryIndex(bucket, entries))
			{
				ref var entry = ref entries[bucket];
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static T UnsafeLoad<T>(this T[] array, int index) => array[index];

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static void UnsafeStore<T>(this T[] array, int index, T value) => array[index] = value;
	...
	public static void Add(this List<char> list, string value)
	{
		...
		value.CopyTo(0, list._items, list._size, value.Length);
	...
	public static unsafe uint GetSizeEstimate<TKey, TValue>(this Dictionary<TKey, TValue> dictionary)
		=> dictionary.GetType().ComputeManagedObjectSizeEstimate()
			+ ((uint)sizeof(Dictionary<TKey, TValue>.Entry) * (uint)(GetEntries(dictionary)?.Length ?? 0))
			+ (sizeof(int) * (uint)(GetBuckets(dictionary)?.Length ?? 0));
