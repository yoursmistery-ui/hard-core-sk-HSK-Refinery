...
{
    public class RKU_GenStep_RockChunks : GenStep
    {
        ...
        private void GrowLowRockFormationFrom(IntVec3 root, Map map)
        {
            ...
            while (true)
            {
                ...
                if (!intVec.InBounds(map) || intVec.GetEdifice(map) != null || intVec.GetFirstItem(map) != null || !map.terrainGrid.TerrainAt(intVec).affordances.Contains(TerrainAffordanceDefOf.Heavy))
                {
                    break;
                }
